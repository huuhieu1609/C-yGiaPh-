  ...
  <div class="landing-page-wrapper">
    ...
    <div class="page-content">
      ...
      <div class="tree-card-wrapper">
        ...
        <div class="tree-viewport" ref="viewport" @wheel.prevent="handleWheel" @mousedown="startPan" @mousemove="doPan"
          ...
          <div class="tree-canvas" :style="canvasStyle">
            ...
            <div class="tree" v-else>
              <ul class="tree-ul">
                <TreeItem v-for="root in treeData" :key="root.id" :member="root" :listDoiTocHo="listDoiTocHo"
                  @view="onView" @show-qr="showQRCard" />
              </ul>
            </div>
          </div>
        </div>
    ...
    <div class="modal fade" id="viewMemberModal" tabindex="-1" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered modal-md">
        <div class="modal-content premium-member-modal-content border-0 overflow-hidden shadow-2xl">
          ...
          <div
            class="modal-header premium-modal-header border-0 bg-royal text-white d-flex align-items-center justify-content-between py-3 px-4"
            style="border-radius:24px 24px 0 0 !important;">
            <h5 class="modal-title fw-extrabold d-flex align-items-center gap-2 text-gold"
          ...
          <div class="modal-body premium-modal-body p-4 text-center position-relative">
            ...
            <div class="modal-avatar-wrapper-premium mb-3 d-inline-block position-relative">
              <img
                ...
                class="rounded-circle border border-4 border-white shadow-lg profile-avatar-modal" width="120"
                height="120" style="object-fit:cover" alt="avatar">
              <span class="status-indicator-modal shadow-sm"
                :class="currentMember.trang_thai === 'Còn sống' ? 'status-alive' : 'status-deceased'">
            ...
            <span class="badge badge-generation-modal mb-3"><i class="bx bx-git-branch"></i> Đời tộc họ thứ {{
              currentMember.doi_thu }}</span>

            <!-- Relationship with logged-in user -->
            <div v-if="isLoggedIn && currentMemberRelationshipLoading"
              class="mb-4 p-3 bg-light-gold border-dashed-gold radius-12 text-center shadow-xs">
            ...
            <div v-else-if="isLoggedIn && currentMemberRelationship"
              class="mb-4 p-3 bg-light-gold border-dashed-gold radius-12 text-center shadow-xs animate__animated animate__fadeIn">
              <span class="text-muted small d-block mb-1 text-uppercase fw-bold font-10 tracking-wider">Mối Quan Hệ Với
                Bạn</span>
            ...
            <div class="row g-3 text-start mt-1">
              <div class="col-6">
                <div class="vital-item-card p-3 radius-12 border bg-light-soft">
                  ...
                  <div class="d-flex align-items-center gap-2">
                    <i class="bx fs-5"
                      :class="currentMember.gioi_tinh === 'Nam' ? 'bx-male-sign text-primary' : 'bx-female-sign text-pink'"></i>
              ...
              <div class="col-6">
                <div class="vital-item-card p-3 radius-12 border bg-light-soft">
                  ...
                  <div class="d-flex align-items-center gap-2">
                    <i class="bx fs-5"
                      :class="currentMember.trang_thai === 'Đã mất' ? 'bx-bookmark-heart text-danger' : 'bx-heart text-success'"></i>
                    <span class="fw-bold font-14"
                      :class="currentMember.trang_thai === 'Đã mất' ? 'text-danger' : 'text-success'">
              ...
              <div class="col-6">
                <div class="vital-item-card p-3 radius-12 border bg-light-soft">
                  ...
                  <div class="d-flex align-items-center gap-2">
                    ...
                    <span class="fw-bold text-dark font-14">{{ currentMember.ngay_sinh ?
                      fmtDate(currentMember.ngay_sinh) : 'Chưa rõ' }}</span>
                  </div>
                </div>
              </div>
              <div class="col-6" v-if="currentMember.trang_thai === 'Đã mất'">
                <div class="vital-item-card p-3 radius-12 border bg-light-soft">
                  ...
                  <div class="d-flex align-items-center gap-2">
                    ...
                    <span class="fw-bold text-dark font-14">{{ currentMember.ngay_mat ? fmtDate(currentMember.ngay_mat)
                      : 'Chưa rõ' }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="col-12" v-if="currentMember.ghi_chu">
                <div
                  class="vital-item-card p-3 radius-12 border bg-light-soft border-start border-4 border-warning-gold">
                  <span class="text-muted small d-block mb-1 text-uppercase fw-bold font-10"><i
                      class="bx bx-book-open"></i> Ghi chép / Ghi chú</span>
          ...
          <div
            class="modal-footer premium-modal-footer bg-light border-0 justify-content-center gap-2 py-3 px-4 flex-wrap">
            <button class="btn btn-premium-action btn-edit-premium" @click="openProposal('edit')">
              <i class="bx bx-edit-alt"></i> Đề xuất sửa
            </button>
            <button v-if="currentMember.loai_quan_he === 'Chính'" class="btn btn-premium-action btn-child-premium"
              @click="openProposal('add_child')">
              <i class="bx bx-plus-circle"></i> Đề xuất con
            </button>
            <button v-if="currentMember.loai_quan_he === 'Chính'" class="btn btn-premium-action btn-spouse-premium"
              @click="openProposal('add_spouse')">
            ...
            <button v-if="isDirectRelative" class="btn btn-premium-action btn-delete-premium"
              style="background: #ef4444 !important; color: white !important;" @click="openLifeStatusModal">
        ...
        <div class="modal-content radius-15 shadow-lg border-0 bg-dark text-white">
          ...
          <div class="modal-body p-4 text-center">
            ...
            <div class="mb-3 text-start" v-if="lifeStatusForm.trang_thai === 'Đã mất'">
              ...
              <input type="date" class="form-control bg-secondary text-white border-0 radius-8"
                v-model="lifeStatusForm.ngay_mat">
            </div>
          </div>
          <div class="modal-footer border-0 justify-content-center">
            <button class="btn btn-light px-4 radius-10" data-bs-dismiss="modal">Hủy</button>
            <button class="btn btn-warning text-dark px-4 radius-10 fw-bold" @click="submitLifeStatus">Cập nhật
              ngay</button>
          </div>
        </div>
      </div>
          ...
          <div class="modal-body p-4">
            <div class="row g-3">
              <div class="col-md-6">
                ...
                <input type="text" class="form-control radius-8 border-2 shadow-none" v-model="proposalForm.ho_ten"
              ...
              <div class="col-md-3">
                ...
                <input type="number" class="form-control radius-8 border-2 shadow-none" v-model="proposalForm.doi_thu"
                  min="1">
              </div>
              <div class="col-md-6"
                v-if="proposalType === 'add_child' && currentMember.spouses && currentMember.spouses.length > 0">
              ...
              <div class="col-md-12">
                ...
                <!-- Hidden Input -->
                <input type="file" ref="avatarFileInput" class="d-none" accept="image/png, image/jpeg, image/jpg"
                ...
                <div class="avatar-upload-widget radius-10 overflow-hidden">
                  ...
                  <div v-else-if="proposalForm.avatar"
                    class="upload-state-box preview-state d-flex align-items-center gap-3 p-3">
                    <img :src="proposalForm.avatar"
                      class="preview-thumbnail rounded-circle border border-2 border-primary shadow"
                      alt="Avatar Preview">
                    <div class="preview-actions text-start">
                      <button type="button" class="btn btn-sm btn-outline-primary radius-8 px-3 me-2"
                  ...
                  <div v-else
                    class="upload-state-box empty-state-box text-center p-4 border-2 border-dashed radius-10 cursor-pointer"
                    @click="triggerFileInput">
              ...
              <div class="col-md-12">
                ...
                <textarea class="form-control radius-8 border-2 shadow-none" rows="3" v-model="proposalForm.ghi_chu"
                  placeholder="Thông tin thêm về thành viên..."></textarea>
              </div>
            </div>
          </div>
    ...
    <div v-if="showQRModal" class="custom-modal-backdrop animate__animated animate__fadeIn" @click.self="closeQRModal"
      ...
      <div
        class="custom-modal-content animate__animated animate__zoomIn p-4 rounded-4 shadow-2xl bg-white position-relative text-center"
        style="max-width: 420px; z-index: 2060;">
        ...
        <div id="qr-card-print"
          ...
          <div class="d-flex align-items-center gap-3 mb-3 text-start">
            <img
              :src="activeMember.avatar || ('https://ui-avatars.com/api/?name=' + activeMember.ho_ten + '&background=d4af37&color=fff')"
              class="rounded-circle border border-2 border-gold shadow-sm" width="55" height="55"
              style="object-fit: cover;">
            <div class="overflow-hidden">
              <h5 class="fw-extrabold text-white mb-0 text-truncate drop-shadow" style="color: white !important;">{{
                activeMember.ho_ten }}</h5>
          ...
          <div
            ...
            <img
              :src="'https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=' + encodeURIComponent(getMemberQRUrl(activeMember))"
        ...
        <div class="d-flex gap-3">
          ...
          <button class="btn btn-gold w-50 py-2 rounded-pill fw-bold text-dark shadow-sm" @click="downloadQRCard"
    ...
    <div v-if="showDeleteProposalModal" class="custom-modal-backdrop animate__animated animate__fadeIn"
      @click.self="closeDeleteProposalModal" style="z-index: 2000 !important;">
      <div
        class="custom-modal-content animate__animated animate__zoomIn p-4 rounded-4 shadow-2xl bg-white position-relative text-center"
        style="max-width: 460px; z-index: 2060;">
        <button class="btn-close-custom position-absolute top-0 end-0 m-3 border-0 bg-transparent"
        ...
        <p class="text-muted small mb-4">Bạn đang tạo đề xuất yêu cầu quản trị viên/đối tác xóa thành viên **{{
          currentMember.ho_ten }}** khỏi cây gia phả.</p>

        <div class="text-start mb-4">
          <label class="form-label fw-bold text-dark small">Lý do đề xuất xóa <span class="text-danger">*</span></label>
          <textarea class="form-control radius-8 border-2 shadow-none" rows="3" v-model="deleteProposalReason"
            placeholder="Nhập lý do chi tiết vì sao cần xóa thành viên này..."></textarea>
        </div>
        ...
        <div class="d-flex gap-3">
          ...
          <button class="btn btn-danger w-50 py-2 rounded-pill fw-bold text-white shadow-sm"
            @click="submitDeleteProposal"
    ...
    <div class="modal fade" id="proposalsHistoryModal" tabindex="-1" aria-hidden="true"
        ...
        <div class="modal-content radius-15 shadow-lg border-0 bg-dark text-white">
          ...
          <div class="modal-body p-4 scrollable-proposals" style="max-height: 480px; overflow-y: auto;">
            ...
            <div v-else class="timeline-list">
              <div v-for="item in proposalsList" :key="item.id" class="proposal-history-item mb-4 p-3 rounded-3"
                ...
                <div class="proposal-body text-white-50 mt-3">
                  ...
                  <div class="target-member mb-2 p-2 rounded bg-white/5 border border-white/5" v-if="item.thanh_vien">
                    <span class="small d-block text-white-50 mb-1">
                      {{ item.type === 'edit' ? 'Thành viên cần chỉnh sửa:' : (item.type === 'add_child' ?
                        'Đề xuất làm con của: ' : 'Đề xuất kết hôn với:') }}
                    </span>
                  ...
                  <div class="p-2 rounded" style="background: rgba(0,0,0,0.2);">
                    <div class="row g-2 text-start small">
                      ...
                      <div class="col-6" v-if="item.data.ngay_sinh">Ngày sinh: <strong class="text-white">{{
                        fmtDate(item.data.ngay_sinh) }}</strong></div>
                      <div class="col-6">Trạng thái: <strong
                          :class="item.data.trang_thai === 'Đã mất' ? 'text-danger' : 'text-success'">{{
                            item.data.trang_thai }}</strong></div>
                      <div class="col-6" v-if="item.data.trang_thai === 'Đã mất' && item.data.ngay_mat">Ngày mất:
                        <strong class="text-white">{{ fmtDate(item.data.ngay_mat) }}</strong>
                      </div>
                      <div class="col-12 mt-1" v-if="item.data.ghi_chu">Ghi chú: <p class="mb-0 text-white-50">{{
                        item.data.ghi_chu }}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="mt-2 p-2 rounded border"
                    :class="item.status === 'approved' ? 'border-success/20 bg-success/5 text-success' : 'border-danger/20 bg-danger/5 text-danger'"
                    v-if="item.note">
                    <span class="d-block small fw-bold mb-1"><i class="bx bx-comment-detail me-1"></i>Phản hồi từ Gia
                      tộc:</span>
...
.status-indicator-modal.status-alive {
  background-color: #10b981;
}

.status-indicator-modal.status-deceased {
