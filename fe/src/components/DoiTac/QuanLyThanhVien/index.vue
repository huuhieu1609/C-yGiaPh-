  ...
  <div class="card shadow-sm border-0 radius-10">
    <div class="card-header bg-white py-3 border-0">
      <h5 class="mb-0 fw-bold text-dark">
        <i class="bx bx-group text-primary"></i> Quản Lý Thành Viên Dòng Họ
      </h5>
    </div>
    <div class="card-body">
      <div v-if="listChiNhanh.length === 0" class="text-center py-5">
        <div class="mb-4 mt-5">
          <i
            class="bx bx-building-house fs-1 text-muted opacity-25"
            style="font-size: 100px !important"
          ></i>
        </div>
        <h4 class="fw-bold text-dark">Dòng Họ Chưa Được Khởi Tạo!</h4>
        <p class="text-muted">
          Bạn cần khởi tạo Dòng Họ (Chi Nhánh) trước khi quản lý thành viên.
        </p>
        <router-link
          to="/doi-tac/dong-ho"
          class="btn btn-primary radius-30 px-5 mt-3 shadow-sm mb-5"
        >
      ...
      <div v-else-if="allMembers.length === 0" class="text-center py-5">
        <div class="mb-4 mt-5">
          <i
            class="bx bx-git-branch fs-1 text-muted opacity-25"
            style="font-size: 100px !important"
          ></i>
        </div>
        <h4 class="fw-bold text-dark">Cây Gia Phả Đang Trống!</h4>
        <p class="text-muted">
          Vui lòng thêm thành viên đầu tiên (Thủy Tổ) tại trang
          <b>Cây Gia Phả</b>.
        </p>
        <router-link
          to="/doi-tac/gia-pha"
        ...
        <div class="row mb-3 g-3">
          <div class="col-md-4">
            <div class="position-relative">
              <input
                type="text"
                class="form-control ps-5 radius-10"
                v-model="searchQuery"
                placeholder="Tìm tên thành viên..."
              />
              <span
                class="position-absolute top-50 translate-middle-y start-0 ms-3 text-secondary"
            ...
            <tbody>
              <tr v-if="filteredMembers.length === 0">
                <td colspan="6" class="text-center py-5 text-muted">
                  ...
                  Vui lòng thêm thành viên (Thủy Tổ) tại trang
                  <b>Cây Gia Phả</b> để bắt đầu.
                </td>
              </tr>
              <tr v-else v-for="item in filteredMembers" :key="item.id">
                ...
                <td class="text-center">
                  <span class="badge bg-light-primary text-primary px-3"
                    >Đời {{ item.doi_thu }}</span
                  >
                </td>
    ...
    <div class="modal fade" id="memberModal" tabindex="-1" aria-hidden="true">
      <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content radius-15 shadow-lg border-0">
          <div
            class="modal-header border-0 text-white radius-top-15"
          ...
          <div class="modal-body p-4">
            <div class="row g-4">
              ...
              <div class="col-md-3">
                ...
                <select
                  class="form-select radius-8 border-2 shadow-none"
          ...
          <div class="modal-footer border-0 p-4 pt-0">
            <button
              type="button"
              class="btn btn-light px-4 radius-8"
              data-bs-dismiss="modal"
            >
              Hủy
            </button>
            <button
              type="button"
              class="btn px-4 radius-8 fw-bold"
  ...
  methods: {
    ...
    loadDoiTocHo() {
      axios
        ...
        .then((res) => {
          if (res.data.status) this.listDoiTocHo = res.data.data;
        });
    },
    loadData() {
      axios
        ...
        .then((res) => {
          if (res.data.status) this.allMembers = res.data.data;
        });
    },
    loadChiNhanh() {
      axios
