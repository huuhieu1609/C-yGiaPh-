    ...
    methods: {
        viewDetail() {
            ...
            if (!isAuthenticated) {
                this.$router.push({
                    path: '/login',
                    query: { redirect: '/dich-vu-goi/chi-tiet' }
                });
                return;
            }
            this.$router.push('/dich-vu-goi/chi-tiet');
        }
    }
}
</script>
...
.package-page {
    min-height: 100vh;
    background: linear-gradient(180deg, #0f172a 0%, #1e293b 45%, #e2e8f0 100%);
...
.package-hero {
    ...
    background:
        ...
        url('@/assets/images/card_product.png');
