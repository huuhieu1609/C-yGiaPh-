            ...
            <div class="card shadow-sm border-0 radius-10 h-100">
                <div class="card-header bg-white py-3 border-0 border-bottom">
                    <h5 class="mb-0 fw-bold text-uppercase" style="color: #00b4d8;">
                        <i class="bx bx-gift me-1"></i> {{ isEditing ? 'Cập Nhật Đóng Góp' : 'Ghi Nhận Đóng Góp Mới' }}
                    </h5>
                </div>
                <div class="card-body p-4">
                    <form @submit.prevent="saveData">
                        <div class="mb-3">
                            ...
                            <select class="form-select radius-8 border-2 shadow-none" v-model="formData.nguoi_dung_id" required>
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-semibold">Nội Dung Đóng Góp / Thanh Toán</label>
            ...
            <div class="card shadow-sm border-0 radius-10 h-100">
                <div class="card-header bg-white py-3 border-0 border-bottom d-flex align-items-center justify-content-between">
                    <h5 class="mb-0 fw-bold text-uppercase" style="color: #333;">
                        <i class="bx bx-list-ul me-1"></i> Danh Sách Đóng Góp & Thanh Toán
                    </h5>
                </div>
                <div class="card-body p-4">
                        ...
                        <table class="table table-bordered table-hover align-middle mb-0">
                            <thead class="text-center text-white" style="background-color: #008bf8;">
                                <tr>
                                    ...
                                    <th width="40%" class="py-3">Nội Dung Chi Tiết</th>
                            ...
                            <tbody>
                                ...
                                <tr v-else-if="filteredList.length === 0">
                                    <td colspan="5" class="text-center py-5 text-muted">
                                        ...
                                        Chưa có dữ liệu đóng góp nào
                                    </td>
                                </tr>
                                <tr v-for="(item, index) in filteredList" :key="item.id">
                                    <td class="text-center fw-bold">{{ index + 1 }}</td>
                                    <td class="fw-semibold">
                                        {{ getUserName(item.nguoi_dung_id) }}
                                        <div class="small text-muted" style="font-size: 11px;">
                                            {{ getUserEmail(item.nguoi_dung_id) }}
                                        </div>
                                    </td>
                                    <td style="white-space: pre-line;">{{ item.noi_dung }}</td>
                                    <td class="text-center">
                                        <button @click="changeStatus(item.id)" 
                                            :class="item.trang_thai === 'Đã duyệt' ? 'btn-success' : 'btn-warning text-dark'" 
                                            class="btn btn-sm radius-8 w-100 fw-bold">
                                            {{ item.trang_thai }}
                                        </button>
                                    </td>
                                    <td class="text-center">
                                        <div class="d-flex justify-content-center gap-2">
                                            <button class="btn btn-sm btn-outline-primary radius-8" type="button" @click="editItem(item)"
                                                title="Sửa">
                                                <i class="bx bx-edit-alt m-0"></i>
                                            </button>
                                            <button class="btn btn-sm btn-outline-danger radius-8" type="button" @click="deleteItem(item.id)"
                                                title="Xóa">
...
export default {
    ...
    computed: {
        filteredList() {
            ...
            return this.listData.filter(item => {
                const userName = this.getUserName(item.nguoi_dung_id).toLowerCase();
    ...
    methods: {
        ...
        getUserName(userId) {
            ...
            return user ? user.ho_ten : 'Hệ thống';
        },
        getUserEmail(userId) {
            const user = this.listUsers.find(u => u.id === userId);
