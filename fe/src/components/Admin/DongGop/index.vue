  ...
  <div class="container-fluid py-4">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
      <div>
        <h1 class="h3 fw-bold text-gradient mb-1" id="page-title">
          Hệ Thống Quản Lý Đóng Góp Công Đức
        </h1>
        <p class="text-muted small">Quản trị toàn bộ các khoản đóng góp quỹ dòng họ, kiểm duyệt giao dịch chuyển khoản
          SePay và ghi nhận thủ công cho tất cả chi nhánh.</p>
      </div>
      <div>
        <button
          class="btn btn-primary-gradient radius-12 px-4 py-2-5 shadow-sm fw-bold border-0 d-flex align-items-center gap-2 btn-hover-scale"
          id="btn-open-modal" @click="openCreateModal">
    ...
    <div class="row g-4 mb-4">
      ...
      <div class="col-12 col-md-3">
        <div
          class="stat-card p-4 radius-20 shadow-sm border-0 d-flex justify-content-between align-items-center bg-card-glow-purple">
          <div class="lh-1">
            <span class="text-uppercase text-muted-custom fw-semibold font-xs tracking-wider d-block mb-2">Tổng Quỹ Công
              Đức</span>
          ...
          <div
            class="card-icon-wrap rounded-circle bg-purple-subtle d-flex align-items-center justify-content-center text-purple">
      ...
      <div class="col-12 col-md-3">
        <div
          class="stat-card p-4 radius-20 shadow-sm border-0 d-flex justify-content-between align-items-center bg-card-glow-teal">
          <div class="lh-1">
            <span class="text-uppercase text-muted-custom fw-semibold font-xs tracking-wider d-block mb-2">Quỹ Tiền
              Mặt</span>
          ...
          <div
            class="card-icon-wrap rounded-circle bg-teal-subtle d-flex align-items-center justify-content-center text-teal">
      ...
      <div class="col-12 col-md-3">
        <div
          class="stat-card p-4 radius-20 shadow-sm border-0 d-flex justify-content-between align-items-center bg-card-glow-blue">
          <div class="lh-1">
            <span class="text-uppercase text-muted-custom fw-semibold font-xs tracking-wider d-block mb-2">Quỹ Chuyển
              Khoản</span>
          ...
          <div
            class="card-icon-wrap rounded-circle bg-blue-subtle d-flex align-items-center justify-content-center text-blue">
      ...
      <div class="col-12 col-md-3">
        <div
          class="stat-card p-4 radius-20 shadow-sm border-0 d-flex justify-content-between align-items-center bg-card-glow-orange">
          <div class="lh-1">
            <span class="text-uppercase text-muted-custom fw-semibold font-xs tracking-wider d-block mb-2">Đang Chờ
              Duyệt</span>
          ...
          <div
            class="card-icon-wrap rounded-circle bg-orange-subtle d-flex align-items-center justify-content-center text-orange">
    ...
    <div class="card shadow-sm border-0 radius-20 bg-card overflow-hidden">
      <div
        class="card-header bg-transparent border-0 py-4 px-4 d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
        ...
        <!-- Filter and Search -->
        <div
          class="d-flex flex-column flex-sm-row align-items-sm-center gap-3 flex-grow-1 max-w-lg-100 justify-content-md-end">
          <!-- Clan / Branch Filter -->
          <div class="branch-filter-wrap">
            <select class="form-select radius-12 border py-2 shadow-none-focus bg-transparent text-muted-custom"
              v-model="filterBranchId" id="filter-branch">
          ...
          <div class="search-wrap flex-grow-1">
            <div class="input-group radius-12 border overflow-hidden search-input-group shadow-none-focus">
              ...
              <input type="text" class="form-control border-0 bg-transparent px-2 py-2 fs-6 shadow-none"
                placeholder="Tìm kiếm đóng góp..." v-model="searchQuery" id="search-input" />
          ...
          <table class="table table-hover align-middle mb-0 custom-modern-table" id="donations-table">
            <thead>
              <tr>
                ...
                <th class="text-muted-custom py-3 font-xs text-uppercase tracking-wider" width="20%">Nội Dung Chi Tiết
                </th>
                <th class="text-muted-custom py-3 font-xs text-uppercase tracking-wider text-center" width="10%">Hình
                  Thức</th>
                <th class="text-muted-custom py-3 font-xs text-uppercase tracking-wider text-center" width="10%">Trạng
                  Thái</th>
                <th class="text-muted-custom py-3 font-xs text-uppercase tracking-wider" width="12%">Thời Gian</th>
                <th class="pe-4 text-muted-custom py-3 font-xs text-uppercase tracking-wider text-end" width="10%">Hành
                  Động</th>
              </tr>
            </thead>
            <tbody>
              ...
              <tr v-for="(item, index) in filteredList" :key="item.id" class="table-row-hover">
                ...
                <td>
                  <div class="d-flex align-items-center gap-3">
                    <div class="avatar-container position-relative">
                      <img
                        v-if="item.nguoi_dung?.avatar && item.nguoi_dung?.avatar !== 'https://dzfullstack.com/assets/images/logo-1.png'"
                        :src="item.nguoi_dung?.avatar" alt="Avatar"
                        class="avatar-img rounded-circle border shadow-sm" />
                      <div v-else
                        class="avatar-fallback rounded-circle bg-purple-subtle text-purple fw-bold d-flex align-items-center justify-content-center border border-purple-subtle shadow-sm">
                        {{ getFallbackInitial(item.nguoi_dung?.ho_ten || 'Hệ thống') }}
                      </div>
                    </div>
                    <div class="lh-1">
                      <strong class="text-main font-sm d-block mb-1">{{ item.nguoi_dung?.ho_ten || 'Thành viên ẩn danh'
                      }}</strong>
                ...
                <td class="max-w-xs text-truncate-custom font-sm text-muted-custom">{{ cleanContent(item.noi_dung) }}
                </td>
                ...
                <td class="pe-4 text-end">
                  <div class="d-inline-flex gap-2">
                    <button class="btn btn-sm btn-icon-glass text-primary radius-10 btn-hover-scale"
                      @click="editItem(item)" title="Chỉnh sửa chi tiết">
                      <i class="bx bx-edit-alt"></i>
                    </button>
                    <button class="btn btn-sm btn-icon-glass text-danger radius-10 btn-hover-scale"
                      @click="deleteItem(item.id)" title="Xóa bản ghi">
    ...
    <div class="modal fade" id="contributionModal" tabindex="-1" aria-hidden="true" ref="modalElement">
      <div class="modal-dialog modal-dialog-centered modal-md">
        <div class="modal-content border-0 radius-24 shadow-lg overflow-hidden bg-card">
          <!-- Modal Header -->
          <div
            class="modal-header py-4 px-4 border-0 d-flex align-items-center justify-content-between modal-header-purple">
            ...
            <button type="button"
              class="btn-close-custom bg-transparent border-0 text-white opacity-75 hover-opacity-100"
          ...
          <form @submit.prevent="saveData">
            <div class="modal-body p-4">
              <!-- Select Branch -->
              <div class="mb-4">
                <label class="form-label fw-bold text-main font-sm mb-2" for="modal-branch">Chọn Chi Nhánh / Dòng Họ
                  <span class="text-danger">*</span></label>
                <select class="form-select radius-12 border-2 py-2-5 shadow-none-focus" id="modal-branch"
                  v-model="tempBranchId" @change="onBranchChange" required>
              ...
              <!-- Select Contributor -->
              <div class="mb-4">
                <label class="form-label fw-bold text-main font-sm mb-2" for="select-user">Người Đóng Góp <span
                    class="text-danger">*</span></label>
                <select class="form-select radius-12 border-2 py-2-5 shadow-none-focus" id="select-user"
                  v-model="formData.nguoi_dung_id" :disabled="!tempBranchId" required>
                  <option :value="null">-- {{ tempBranchId ? 'Chọn thành viên trong chi nhánh' :
                    'Vui lòng chọn chi nhánh trước' }} --</option>
              ...
              <!-- Select Payment Type -->
              <div class="mb-4">
                <label class="form-label fw-bold text-main font-sm mb-2" for="select-type">Hình Thức Đóng Góp <span
                    class="text-danger">*</span></label>
                <select class="form-select radius-12 border-2 py-2-5 shadow-none-focus" id="select-type"
                  v-model="tempType" required>
              ...
              <!-- Input Amount -->
              <div class="mb-4">
                <label class="form-label fw-bold text-main font-sm mb-2" for="input-amount">Số Tiền (VNĐ) <span
                    class="text-danger">*</span></label>
                <div class="input-group radius-12 border-2 overflow-hidden shadow-none-focus-wrap">
                  ...
                  <input type="number" id="input-amount" class="form-control border-0 py-2-5 shadow-none"
                    placeholder="Nhập số tiền đóng góp (ví dụ: 500000)" v-model.number="tempAmount" min="1000"
                    required />
                </div>
              ...
              <!-- Select Status -->
              <div class="mb-4">
                <label class="form-label fw-bold text-main font-sm mb-2" for="select-status">Trạng Thái Phê Duyệt <span
                    class="text-danger">*</span></label>
                <select class="form-select radius-12 border-2 py-2-5 shadow-none-focus" id="select-status"
                  v-model="formData.trang_thai" required>
              ...
              <!-- Notes Textarea -->
              <div class="mb-4">
                <label class="form-label fw-bold text-main font-sm mb-2" for="input-notes">Mục Đích / Ghi Chú Đóng Góp
                  <span class="text-danger">*</span></label>
                <textarea rows="4" id="input-notes" class="form-control radius-12 border-2 py-2-5 shadow-none-focus"
                  placeholder="Nhập lý lý do đóng tiền (ví dụ: Đóng góp tiền mặt hỗ trợ ngày giỗ tổ...)"
                  v-model="tempNotes" required></textarea>
              </div>
            </div>

            <!-- Modal Footer -->
            <div class="modal-footer py-3 px-4 border-0 bg-light d-flex justify-content-end gap-2">
              <button type="button" class="btn btn-light radius-12 px-4 py-2 fw-semibold" data-bs-dismiss="modal"
