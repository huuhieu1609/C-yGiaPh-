<template>
    <div class="dong-gop-management-wrapper">
        <!-- Page Header -->
        <div class="page-header mb-4 d-flex align-items-center justify-content-between flex-wrap gap-3">
            <div class="d-flex align-items-center gap-3">
                <div class="page-icon">
                    <i class='bx bx-wallet' style="color: #8b5cf6;"></i>
                </div>
                <div>
                    <h4 class="mb-0 fw-bold theme-text-main page-title text-gradient-gold">Quản Lý Đóng Góp</h4>
                    <p class="mb-0 text-secondary small mt-1">Theo dõi, phê duyệt và thống kê các khoản đóng góp quỹ dòng họ hoặc thanh toán dịch vụ mua gói của đối tác</p>
                </div>
            </div>
            <div class="d-flex align-items-center gap-3">
        ...
        <div v-if="!selectedPartner" class="card shadow-sm border-0 radius-10 p-5 text-center bg-white">
            <div class="empty-state-icon mb-4">
                <i class="bx bx-wallet fs-1 text-warning opacity-75 animate__animated animate__pulse animate__infinite" style="font-size: 60px;"></i>
            </div>
            <h4 class="fw-bold text-dark mb-3">Chưa Chọn Đối Tác Quản Lý Đóng Góp</h4>
            <p class="text-muted small mx-auto mb-4" style="max-width: 480px; font-size: 14px; line-height: 1.6;">
                Vui lòng lựa chọn một tài khoản đối tác từ danh sách để tải dữ liệu đóng góp/thanh toán và thực hiện quản lý tương ứng.
            </p>
            <div>
                ...
                <div class="card luxury-panel border-0 shadow-sm h-100">
                    ...
                    <div class="card-body p-4">
                        <form @submit.prevent="saveData">
                            <div class="mb-4">
                                <label class="form-label fw-bold text-secondary-custom">Người Đóng Góp</label>
                                <input type="text" class="form-control premium-input radius-10 border-2 shadow-none" :value="selectedPartner ? (selectedPartner.nguoi_dung?.ho_ten || selectedPartner.ten_goi) + ' (' + selectedPartner.nguoi_dung?.email + ')' : ''" disabled>
                            </div>
                            
                            <div class="mb-4">
                                <label class="form-label fw-bold text-secondary-custom">Nội Dung Đóng Góp / Thanh Toán</label>
                                <textarea rows="5" class="form-control premium-input radius-10 border-2 shadow-none"
                                    placeholder="VD: DONGGOP HIEU | Số tiền: 500.000 VNĐ cho quỹ dòng họ hoặc chuyển khoản MUAGOI..." 
                                    v-model="formData.noi_dung" required></textarea>
                            </div>
                            ...
                            <div class="d-grid gap-2 d-md-flex justify-content-md-end mt-5">
                                ...
                                <button type="submit" class="btn btn-filter-submit text-white radius-30 px-4 fw-bold shadow-sm">
                                    {{ isEditing ? 'Cập Nhật' : 'Lưu Đóng Góp' }}
                                </button>
                            </div>
                        </form>
                    </div>
                ...
                <div class="card luxury-panel border-0 shadow-sm h-100">
                    <div class="card-header bg-transparent py-4 border-0 border-bottom border-light-subtle d-flex align-items-center justify-content-between flex-wrap gap-3">
                        <h5 class="mb-0 fw-bold panel-title text-dark">
                            <i class="bx bx-list-ul me-2 text-warning"></i> Danh Sách Đóng Góp & Thanh Toán
                        </h5>
                    </div>
                    <div class="card-body p-4">
                        <div class="input-group mb-4 radius-10 overflow-hidden border-2 search-box-premium shadow-sm">
                            <input type="text" class="form-control border-0 shadow-none ps-4 bg-transparent" placeholder="Tìm kiếm đóng góp..." v-model="searchQuery">
                            ...
                            <table class="table modern-table align-middle mb-0">
                                <thead>
                                    <tr>
                                        ...
                                        <th width="45%">Nội Dung Chi Tiết</th>
                                ...
                                <tbody>
                                    ...
                                    <tr v-else-if="filteredList.length === 0">
                                        <td colspan="5" class="text-center py-5 text-muted">
                                            ...
                                            Chưa có dữ liệu đóng góp nào cho đối tác này
                                        </td>
                                    </tr>
                                    <tr v-for="(item, index) in filteredList" :key="item.id">
                                        <td class="text-center fw-bold">{{ index + 1 }}</td>
                                        <td class="fw-semibold text-dark">
                                            {{ selectedPartner ? (selectedPartner.nguoi_dung?.ho_ten || selectedPartner.ten_goi) : 'Hệ thống' }}
                                            <div class="small text-muted" style="font-size: 11px;">
                                                {{ selectedPartner ? selectedPartner.nguoi_dung?.email : '' }}
                                            </div>
                                        </td>
                                        <td style="white-space: pre-line;" class="text-secondary small">{{ item.noi_dung }}</td>
                                        <td class="text-center">
                                            <button @click="changeStatus(item.id)" 
                                                :class="item.trang_thai === 'Đã duyệt' ? 'btn-status-active' : 'btn-status-locked'" 
...
export default {
    ...
    computed: {
        ...
        filteredList() {
            if (!this.selectedPartner || !this.selectedPartner.id_nguoi_dung) return [];
            
            // Filter list by selected partner's user ID
            let base = this.listData.filter(item => item.nguoi_dung_id === this.selectedPartner.id_nguoi_dung);

            if (!this.searchQuery) return base;
            const q = this.searchQuery.toLowerCase();
            return base.filter(item => {
                const content = item.noi_dung ? item.noi_dung.toLowerCase() : '';
    ...
    mounted() {
        ...
        if (saved) {
            ...
            this.formData.nguoi_dung_id = item.id_nguoi_dung;
        }
        this.loadData();
        this.loadPartners();
    },
    methods: {
                    ...
                    if (res.data.status) {
                        ...
                        if (saved) {
                            ...
                            if (fresh) {
                                ...
                                this.formData.nguoi_dung_id = fresh.id_nguoi_dung;
        ...
        saveData() {
            ...
            this.formData.nguoi_dung_id = this.selectedPartner.id_nguoi_dung;
        ...
        deleteItem(id) {
            if (confirm('Bạn có chắc chắn muốn xóa đóng góp này không?')) {
                ...
                .catch(err => {
                    toastr.error('Lỗi khi xóa đóng góp!');
                });
            }
        },
        changeStatus(id) {
            axios.post('http://127.0.0.1:8000/api/dong-gop/status', { id }, this.getHeaders())
            .then(res => {
                if (res.data.status) {
                    toastr.success(res.data.message);
                    this.loadData();
            ...
            .catch(err => {
                toastr.error('Lỗi khi thay đổi trạng thái đóng góp!');
            });
        },
        resetForm() {
            ...
            this.formData = {
                id: null,
                nguoi_dung_id: this.selectedPartner ? this.selectedPartner.id_nguoi_dung : null,
                noi_dung: '',
                trang_thai: 'Chờ duyệt'
            };
        }
