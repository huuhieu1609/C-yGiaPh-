            ...
            <div class="card shadow-sm border-0 radius-10 h-100">
                <div class="card-header bg-white py-3 border-0 border-bottom">
                    <h5 class="mb-0 fw-bold text-uppercase" style="color: #00b4d8;">
                        ...
                        {{ isEditing ? 'Cập Nhật Nhà Thờ Họ' : 'Thêm Nhà Thờ Họ Mới' }}
                    </h5>
                </div>
                <div class="card-body p-4">
                    <form @submit.prevent="saveData">
                        <div class="mb-3">
                            <label class="form-label fw-semibold">Tên Nhà Thờ</label>
                            <input type="text" class="form-control radius-8 border-2 shadow-none"
                                placeholder="Nhập tên nhà thờ..." v-model="formData.ten_nha_tho" required />
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea rows="4" class="form-control radius-8 border-2 shadow-none"
                                placeholder="Nhập mô tả chi tiết..." v-model="formData.mo_ta"></textarea>
                        </div>
                        ...
                        <table class="table table-bordered table-hover align-middle mb-0">
                            <thead class="text-center text-white" style="background-color: #008bf8;">
                                <tr>
                                    ...
                                    <th width="30%" class="py-3">Tên Nhà Thờ</th>
                                    <th width="30%" class="py-3">Địa Chỉ</th>
                            ...
                            <tbody>
                                ...
                                <tr v-else-if="filteredList.length === 0">
                                    <td colspan="5" class="text-center py-5 text-muted">
                                        ...
                                        Chưa có dữ liệu nhà thờ họ
                                    </td>
                                </tr>
                                <tr v-for="(item, index) in filteredList" :key="item.id">
                                    <td class="text-center fw-bold">{{ index + 1 }}</td>
                                    <td class="fw-semibold">{{ item.ten_nha_tho }}</td>
                                    <td>{{ item.dia_chi }}</td>
                                    <td>{{ item.mo_ta || '---' }}</td>
    ...
    computed: {
        filteredList() {
            ...
            return this.listData.filter(item =>
                item.ten_nha_tho.toLowerCase().includes(q) ||
    ...
    methods: {
        ...
        loadData() {
            ...
            axios.get('http://127.0.0.1:8000/api/nha-tho-ho/get-data', this.getHeaders())
                ...
                .catch(err => {
                    console.error(err);
                    toastr.error('Không thể tải danh sách nhà thờ họ');
                })
        ...
        saveData() {
            ...
            axios.post(url, this.formData, this.getHeaders())
                ...
                .catch(err => {
                    const message = err.response?.data?.message || 'Lỗi khi lưu nhà thờ họ';
        ...
        deleteItem(id) {
            if (!confirm('Bạn có chắc chắn muốn xóa nhà thờ họ này?')) return;
            axios.post('http://127.0.0.1:8000/api/nha-tho-ho/delete', { id }, this.getHeaders())
                ...
                .catch(err => {
                    const message = err.response?.data?.message || 'Lỗi khi xóa nhà thờ họ';
                    toastr.error(message);
                });
        },
        resetForm() {
