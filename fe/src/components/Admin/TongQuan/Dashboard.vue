                ...
                <div class="card luxury-panel border-0 shadow-sm">
                    <div class="card-header bg-transparent py-4 border-0 d-flex align-items-center justify-content-between flex-wrap gap-3">
                        ...
                        <div class="custom-chart-legend d-flex align-items-center gap-3">
                            <div 
                                class="legend-item" 
                                :class="{ 'legend-item-disabled': datasetVisibility[0] === false }"
                            ...
                            >
                                ...
                                <span class="legend-value-badge bg-blue-subtle">{{ formatNumber(metrics.total_members) }}</span>
                            </div>
                            <div 
                                class="legend-item" 
                                :class="{ 'legend-item-disabled': datasetVisibility[1] === false }"
                                @click="toggleDatasetVisibility(1)"
                            >
                                <span class="legend-color-dot dot-pink"></span>
                                <span class="legend-label-text">Cây Gia Phả</span>
                                <span class="legend-value-badge bg-pink-subtle">{{ formatNumber(metrics.total_trees) }}</span>
                            </div>
                        </div>
                    </div>
                    <div class="card-body p-4 pt-0">
                        <!-- Premium Filter Toolbar -->
                        <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4 pb-3 border-bottom border-light-subtle">
                            <!-- Quick Range Buttons -->
                            <div class="btn-group premium-btn-group" role="group">
                                <button 
                                    type="button" 
                                    class="btn premium-filter-btn" 
                            ...
                            <!-- Custom Date Inputs -->
                            <div v-if="currentFilter === 'custom'" class="custom-date-inputs d-flex align-items-center gap-2 animate-fade-in">
                                <div class="input-group input-group-sm">
                                    <span class="input-group-text bg-light border-light-subtle text-secondary small-label">Từ</span>
                                    <input type="date" v-model="startDate" class="form-control premium-date-input" :max="endDate || todayDate">
                                </div>
                                <div class="input-group input-group-sm">
                                    <span class="input-group-text bg-light border-light-subtle text-secondary small-label">Đến</span>
                                    <input type="date" v-model="endDate" class="form-control premium-date-input" :min="startDate" :max="todayDate">
                                </div>
                                <button class="btn btn-sm btn-filter-submit" @click="applyCustomDateRange" :disabled="!startDate || !endDate">
                        ...
                        <div class="chart-summary-stats d-flex gap-4 mb-4 pb-3 border-bottom border-light-subtle flex-wrap">
                            ...
                            <div class="summary-stat-box border-start ps-4">
                                ...
                                <strong class="summary-val text-secondary small" style="font-size: 13px; line-height: 24px;">
                                ...
                                <tbody>
                                    ...
                                    <tr v-for="item in recentUpdates" :key="item.id">
                                        ...
                                        <td class="text-center">
                                            <span class="badge status-badge" :class="item.trang_thai === 'Còn sống' ? 'badge-approved' : 'badge-rejected'">
                                                {{ item.trang_thai || 'Chưa rõ' }}
                                            </span>
                                        </td>
                                        <td class="text-center">
                                            <div class="d-flex justify-content-center gap-2">
                                                <router-link to="/admin/gia-pha" class="btn btn-action-edit" title="Đi tới Quản lý"><i class='bx bx-cog'></i></router-link>
                                            </div>
                                        </td>
                                    </tr>
                                </tbody>
                        ...
                        <div class="traffic-source-container d-flex flex-column gap-4">
                            <div class="traffic-source d-flex align-items-center">
                                ...
                                <div class="ms-3">
                                    ...
                                    <h6 class="activity-desc mb-0 fw-bold text-dark">{{ formatNumber(systemActivities.new_users_today) }} tài khoản hệ thống</h6>
                                </div>
                            </div>
                            <div class="traffic-source d-flex align-items-center">
                                ...
                                <div class="ms-3">
                                    ...
                                    <h6 class="activity-desc mb-0 fw-bold text-dark">{{ formatNumber(systemActivities.images_uploaded) }} tệp tin ảnh lưu trữ</h6>
                                </div>
                            </div>
                            <div class="traffic-source d-flex align-items-center">
                                ...
                                <div class="ms-3">
                                    ...
                                    <h6 class="activity-desc mb-0 fw-bold text-dark">{{ formatNumber(systemActivities.notifications) }} bản tin được phát đi</h6>
                                </div>
                            </div>
                        </div>
                    </div>
...
.stat-desc i { font-size: 13px; vertical-align: -1px; }
...
.item-total .stat-icon-box { background: #e0f2fe; color: #0284c7; border-color: rgba(2, 132, 199, 0.1); }
.item-tree .stat-icon-box { background: #fff5f7; color: #db2777; border-color: rgba(219, 39, 119, 0.1); }
.item-donation .stat-icon-box { background: #f0fdf4; color: #059669; border-color: rgba(5, 150, 105, 0.1); }
.item-digitize .stat-icon-box { background: #fef3c7; color: #d97706; border-color: rgba(217, 119, 6, 0.1); }

/* ─── LUXURY LIGHT TABLE SYSTEM ─── */
.modern-table { width: 100%; }
.modern-table thead { background: #f4f5f7; }
...
.modern-table tbody tr:hover { background-color: #fdfdfd; }
...
.badge-approved { background-color: #f0fdf4 !important; color: #16a34a !important; border: 1px solid rgba(22, 163, 74, 0.15); }
.badge-pending { background-color: #fff7ed !important; color: #ea580c !important; border: 1px solid rgba(234, 88, 12, 0.15); }
.badge-rejected { background-color: #fef2f2 !important; color: #ef4444 !important; border: 1px solid rgba(239, 68, 68, 0.15); }
...
.custom-dropdown-trigger:hover { color: #111827; }
.luxury-dropdown {
    background: #ffffff;
    border-radius: 12px;
    padding: 6px;
...
.box-indigo { background: #eef2ff; color: #4f46e5; }
.box-emerald { background: #f0fdf4; color: #059669; }
...
.bg-pink-subtle { background-color: #fff1f2; color: #be123c; }
