    ...
    <div class="main-panel">
        ...
        <div class="panel-actions">
          <div class="filter-group">
            <button v-for="f in filters" :key="f.value" class="filter-pill"
              :class="{ active: activeFilter === f.value }" @click="setFilter(f.value)">{{ f.label }}</button>
          </div>
      ...
      <transition name="slide-down">
        <div class="bulk-bar" v-if="selectedIds.length > 0">
          <span class="bulk-count"><i class="bx bx-checkbox-checked me-1"></i>Đã chọn <strong>{{ selectedIds.length
              }}</strong> tài khoản</span>
      ...
      <div class="table-wrap">
        <table class="acc-table">
          <thead>
            <tr>
              <th style="width:42px" class="text-center">
                <input type="checkbox" class="tbl-check" :checked="allSelected" @change="toggleSelectAll"
          ...
          <tbody>
            ...
            <tr v-else v-for="(acc, idx) in accounts" :key="acc.id" class="acc-row"
              :class="{ 'row-deleted': acc.deleted_at, 'row-selected': selectedIds.includes(acc.id) }">
              ...
              <td>
                <div class="acc-identity">
                  <div class="acc-avatar" :style="{ background: avatarColor(acc.ho_ten) }">{{ initials(acc.ho_ten) }}
                  </div>
                  <div class="acc-info">
              ...
              <td class="text-center">
                <div class="actions-cell">
                  ...
                  <button class="act-btn act-edit" @click="openEdit(acc)" title="Chỉnh sửa"
                    :disabled="!!acc.deleted_at">
                  ...
                  <div class="act-more-wrap" v-click-outside="() => closeMenu(acc.id)">
                    ...
                    <div class="dropdown-menu-custom" :class="{ show: openMenuId === acc.id }">
                      <template v-if="!acc.deleted_at">
                        ...
                        <button class="dd-item" v-if="acc.trang_thai !== 'Khóa'"
                          @click="doLock(acc); closeMenu(acc.id)">
        ...
        <div class="modal-box modal-lg">
          ...
          <div class="modal-body-custom" v-if="detailData">
            ...
            <div class="detail-grid">
              <div class="detail-item"><label>Số điện thoại</label><span>{{ detailData.account.so_dien_thoai || '—'
                  }}</span></div>
              <div class="detail-item"><label>Trạng thái</label><span>{{ detailData.account.trang_thai }}</span></div>
              <div class="detail-item"><label>Ngày tạo</label><span>{{ formatDate(detailData.account.created_at)
                  }}</span>
              </div>
              <div class="detail-item" v-if="detailData.account.doi_tac"><label>Gói hiện tại</label><span>{{
                detailData.account.doi_tac.ten_goi }}</span></div>
              ...
              <div class="detail-item" v-if="detailData.account.doi_tac"><label>Trạng thái gói</label><span>{{
                detailData.account.doi_tac.trang_thai }}</span></div>
            </div>
            <div v-if="detailData.logs && detailData.logs.length" class="detail-logs">
              <h6 class="fw-bold mb-3"><i class="bx bx-history me-2 text-warning"></i>Lịch sử hoạt động (20 gần nhất)
              </h6>
        ...
        <div class="modal-box modal-sm">
          ...
          <div class="modal-body-custom">
            ...
            <div class="modal-footer-btns">
              ...
              <button class="btn-warning-custom" @click="submitResetPwd"
            ...
            <div class="form-group-custom mt-3">
              ...
              <select class="form-inp" v-model="upgradeForm.ten_goi" @change="onPackageChange">
                ...
                <option v-for="pkg in availablePackages" :key="pkg.id" :value="pkg.ten_goi">
                  {{ pkg.ten_goi }} — {{ formatCurrency(pkg.gia_ca) }} / {{ pkg.thoi_han_nam }} năm
                </option>
              </select>
            </div>
            <div class="form-row-2">
...
.bulk-unlock:hover:not(:disabled) {
  background: #059669;
  color: #fff;
}
...
.bulk-restore:hover:not(:disabled) {
  background: #4f46e5;
  color: #fff;
}
...
.skeleton-bar {
  height: 20px;
  border-radius: 8px;
  background: linear-gradient(90deg, var(--input-bg, #f1f5f9) 25%, rgba(0, 0, 0, 0.04) 50%, var(--input-bg, #f1f5f9) 75%);
...
.str-medium .pwd-label {
  color: #f59e0b;
}

.str-strong .pwd-label {
  color: #10b981;
}
...
.btn-gold-custom:hover:not(:disabled) {
  transform: translateY(-1px);
}

.btn-primary-custom:disabled,
.btn-warning-custom:disabled,
.btn-gold-custom:disabled,
