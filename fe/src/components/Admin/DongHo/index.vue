<template>
    <div class="dong-ho-management-wrapper">
        <!-- Page Header -->
        <div class="page-header mb-4 d-flex align-items-center justify-content-between flex-wrap gap-3">
            <div class="d-flex align-items-center gap-3">
                <div class="page-icon">
                    <i class='bx bx-sitemap' style="color: #059669;"></i>
                </div>
                <div>
                    <h4 class="mb-0 fw-bold theme-text-main page-title text-gradient-gold">Quản Lý Dòng Họ</h4>
                    <p class="mb-0 text-secondary small mt-1">Cấu hình các chi nhánh dòng họ và liên kết tài khoản đối tác đại diện</p>
                </div>
            </div>
            <div class="d-flex align-items-center gap-3">
        ...
        <div v-if="!selectedPartner" class="card shadow-sm border-0 radius-10 p-5 text-center bg-white">
            <div class="empty-state-icon mb-4">
                <i class="bx bx-sitemap fs-1 text-warning opacity-75 animate__animated animate__pulse animate__infinite" style="font-size: 60px;"></i>
            </div>
            <h4 class="fw-bold text-dark mb-3">Chưa Chọn Đối Tác Quản Lý Dòng Họ</h4>
            <p class="text-muted small mx-auto mb-4" style="max-width: 480px; font-size: 14px; line-height: 1.6;">
                Vui lòng lựa chọn một tài khoản đối tác từ danh sách để tải dữ liệu dòng họ và thực hiện quản lý tương ứng.
            </p>
            <div>
                                ...
                                <tbody>
                                    ...
                                    <tr v-else-if="filteredList.length === 0">
                                        <td colspan="6" class="text-center py-5 text-muted">
                                            ...
                                            Chưa có chi nhánh dòng họ nào được khởi tạo hoặc liên kết với đối tác này
                                        </td>
                                    </tr>
                                    <tr v-for="(item, index) in filteredList" :key="item.id">
                                        ...
                                        <td class="text-dark small fw-semibold">
                                            {{ selectedPartner ? (selectedPartner.nguoi_dung?.ho_ten || selectedPartner.ten_goi) : '---' }}
                                        </td>
...
export default {
    ...
    data() {
        return {
            listData: [],
            listPartners: [], // Partners list for selection
            listUsers: [],     // Users list for fallback
    ...
    computed: {
        ...
        filteredList() {
            ...
            // Filter by currently selected partner's Chi Nhanh
            let base = this.listData.filter(item => item.id == this.selectedChiNhanh);
    ...
    mounted() {
        ...
        if (saved) {
            ...
            this.formData.id_nguoi_dung = item.id_nguoi_dung;
        }
        this.loadData();
        this.loadPartners();
    },
    methods: {
                    ...
                    if (res.data.status) {
                        ...
                        if (saved) {
                            ...
                            if (fresh) {
                                ...
                                this.formData.id_nguoi_dung = fresh.id_nguoi_dung;
        ...
        clearPartnerSelection() {
            ...
            this.formData.id_nguoi_dung = null;
            this.showPartnerSelectorModal = false;
        ...
        saveData() {
            ...
            this.formData.id_nguoi_dung = this.selectedPartner.id_nguoi_dung;
        ...
        resetForm() {
            ...
            this.formData = {
                ...
                ten_chi: '',
                dia_chi: '',
                id_nguoi_dung: this.selectedPartner ? this.selectedPartner.id_nguoi_dung : null,
                trang_thai: 'Hoạt động'
            };
        }
    }
